...
<div class="previewer {classesBase}">
		...
		{:else}
			...
			<RadioGroup>
				<RadioItem bind:group={tabView} name="view" value="preview" title="Preview"
					><i class="fa-solid fa-eye text-sm" /></RadioItem
				>
				<RadioItem bind:group={tabView} name="view" value="code" title="Code"
					><i class="fa-solid fa-code text-sm" /></RadioItem
				>
			</RadioGroup>
			<div class="flex justify-between gap-4">
				...
				<!-- Toggle Swatches -->
				<button
					class="btn-icon {swatches ? 'variant-filled' : 'variant-soft'}"
	...
	{#if tabView === 'preview'}
		...
		{#if $$slots.footer}<footer class="previewer-footer {classesFooter}">
				<slot name="footer" />
			</footer>{/if}
